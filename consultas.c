...
int main(int argc, char **argv)
{
	...
	err=mysql_query (conn, "SELECT Jug_Part.ID, Jug_Part.Partida, Partidas.Dinero FROM (Jugador, Partidas,Jug_Part) "
					 "WHERE Jugador.Username = 'Pau' "
					 "AND Jugador.ID = Jug_Part.ID"
					 "AND Partidas.ID = Jug_Part.ID;");
	if (err!=0) {
